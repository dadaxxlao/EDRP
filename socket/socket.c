...
static int validate_socket_config(const struct socket_config *config) {
    if (config == NULL) {
        SOCKET_LOG(LOG_ERROR, "Invalid config pointer");
        return SOCKET_ERROR_INVALID;
    }

    // 验证最大文件描述符数
    if (config->max_fds <= 0 || config->max_fds > 65535) {
        SOCKET_LOG(LOG_ERROR, "Invalid max_fds value: %d (should be between 1 and 65535)", 
                  config->max_fds);
        return SOCKET_ERROR_INVALID;
    }
    ...
    if (config->ring_size <= 0 || config->ring_size > 65535 || 
        ...
        SOCKET_LOG(LOG_ERROR, "Invalid ring_size value: %d (should be power of 2 and between 1 and 65535)", 
                  config->ring_size);
        return SOCKET_ERROR_INVALID;
    }
    ...
    if (config->timeout_sec <= 0 || config->timeout_sec > 3600) {  // 最大1小时
        SOCKET_LOG(LOG_ERROR, "Invalid timeout value: %d (should be between 1 and 3600)", 
                  config->timeout_sec);
        return SOCKET_ERROR_INVALID;
    }
    ...
    if (config->log_level < SOCKET_LOG_ERROR || config->log_level > SOCKET_LOG_DEBUG) {
        SOCKET_LOG(LOG_ERROR, "Invalid log level: %d", config->log_level);
        return SOCKET_ERROR_INVALID;
    }
...
int socket_set_config(const struct socket_config *config) {
    ...
    if (max_used_fd >= config->max_fds) {
        ...
        SOCKET_LOG(LOG_ERROR, "Cannot reduce max_fds: active fd %d exceeds new limit %d",
    ...
    SOCKET_LOG(LOG_INFO, "Socket configuration updated: max_fds=%d, ring_size=%d, timeout=%d, log_level=%d",
...
void socket_set_log_level(int level) {
    ...
    if (level < SOCKET_LOG_ERROR || level > SOCKET_LOG_DEBUG) {
        SOCKET_LOG(LOG_WARNING, "Invalid log level: %d, using default INFO level", level);
    ...
    SOCKET_LOG(LOG_INFO, "Log level set to %d", level);
}
...
static int allocate_fd(void) {
    ...
    for (int fd = 3; fd < g_config.max_fds; fd++) {
        ...
        if ((fd_bitmap[byte] & (1 << bit)) == 0) {
            ...
            pthread_mutex_unlock(&g_config_mutex);
            SOCKET_LOG(LOG_DEBUG, "Allocated fd=%d", fd);
            return fd;
        }
    }
    
    pthread_mutex_unlock(&g_config_mutex);
    SOCKET_LOG(LOG_ERROR, "No available file descriptors");
...
void socket_stats(struct socket_statistics *stats) {
    ...
    SOCKET_LOG(LOG_INFO, "Socket statistics: used_fds=%d, total_fds=%d",
...
static void release_fd(int fd) {
    if (fd < 3 || fd >= g_config.max_fds) {
        SOCKET_LOG(LOG_WARNING, "Invalid file descriptor: %d", fd);
        return;
    }
    ...
    SOCKET_LOG(LOG_DEBUG, "Released fd=%d", fd);
}
...
static void* get_context(int sockfd) {
    if (sockfd < 3 || sockfd >= g_config.max_fds) {
        SOCKET_LOG(LOG_ERROR, "Invalid socket descriptor: fd=%d", sockfd);
        return NULL;
    }
    ...
    if ((fd_bitmap[byte] & (1 << bit)) == 0) {
        pthread_mutex_unlock(&g_config_mutex);
        SOCKET_LOG(LOG_ERROR, "Socket descriptor not allocated: fd=%d", sockfd);
        return NULL;
    }
    pthread_mutex_unlock(&g_config_mutex);
    ...
    SOCKET_LOG(LOG_ERROR, "No context found for socket descriptor: fd=%d", sockfd);
    return NULL;
}
...
int socket(int domain, int type, int protocol) {
    SOCKET_LOG(LOG_DEBUG, "Creating socket: domain=%d, type=%d, protocol=%d", domain, type, protocol);
    ...
    if (domain != AF_INET) {
        SOCKET_LOG(LOG_ERROR, "Unsupported domain: %d", domain);
        errno = EAFNOSUPPORT;
        return SOCKET_ERROR_INVALID;
    }

    if (type != SOCK_DGRAM && type != SOCK_STREAM) {
        SOCKET_LOG(LOG_ERROR, "Unsupported socket type: %d", type);
        errno = EPROTONOSUPPORT;
        return SOCKET_ERROR_INVALID;
    }

    if (protocol != 0 && protocol != IPPROTO_UDP && protocol != IPPROTO_TCP) {
        SOCKET_LOG(LOG_ERROR, "Unsupported protocol: %d", protocol);
        errno = EPROTONOSUPPORT;
        return SOCKET_ERROR_INVALID;
    }
    ...
    int fd = allocate_fd();
    if (fd < 0) {
        SOCKET_LOG(LOG_ERROR, "Failed to allocate file descriptor");
        errno = EMFILE;
        return fd;  // 返回错误码
    }
    ...
    if (type == SOCK_DGRAM) {
        ...
        if (!ctx) {
            SOCKET_LOG(LOG_ERROR, "Failed to allocate UDP context");
            release_fd(fd);
            errno = ENOMEM;
            return SOCKET_ERROR_NOMEM;
        }
        ...
        if (!ctx->sndbuf || !ctx->rcvbuf) {
            SOCKET_LOG(LOG_ERROR, "Failed to create ring buffers for UDP socket");
        ...
        if (pthread_mutex_init(&ctx->mutex, NULL) != 0 || 
            ...
            SOCKET_LOG(LOG_ERROR, "Failed to initialize synchronization primitives");
            if (ctx->sndbuf) rte_ring_free(ctx->sndbuf);
            if (ctx->rcvbuf) rte_ring_free(ctx->rcvbuf);
            rte_free(ctx);
            release_fd(fd);
        ...
        // 添加到全局列表
        LL_ADD(ctx, lhost);
        SOCKET_LOG(LOG_INFO, "Created UDP socket with fd=%d", fd);
        
    } else if (type == SOCK_STREAM) {
        // 创建TCP流
        struct ng_tcp_stream *stream = rte_malloc("tcp_stream", sizeof(struct ng_tcp_stream), 0);
        if (!stream) {
            SOCKET_LOG(LOG_ERROR, "Failed to allocate TCP stream");
            release_fd(fd);
            errno = ENOMEM;
            return SOCKET_ERROR_NOMEM;
        }
        ...
        if (!stream->sndbuf || !stream->rcvbuf) {
            SOCKET_LOG(LOG_ERROR, "Failed to create ring buffers for TCP socket");
        ...
        if (!table) {
            SOCKET_LOG(LOG_ERROR, "Failed to get TCP table instance");
            if (stream->sndbuf) rte_ring_free(stream->sndbuf);
            if (stream->rcvbuf) rte_ring_free(stream->rcvbuf);
        ...
        SOCKET_LOG(LOG_INFO, "Created TCP socket with fd=%d", fd);
    }
...
int bind(int sockfd, const struct sockaddr *addr, socklen_t addrlen) {
    SOCKET_LOG(LOG_DEBUG, "Binding socket fd=%d", sockfd);
    ...
    if (saddr->sin_family != AF_INET) {
        SOCKET_LOG(LOG_ERROR, "Unsupported address family: %d", saddr->sin_family);
        errno = EAFNOSUPPORT;
        return SOCKET_ERROR_INVALID;
    }
    ...
    if (check_port_in_use(saddr->sin_port)) {
        SOCKET_LOG(LOG_ERROR, "Port %d already in use", ntohs(saddr->sin_port));
    ...
    if (((struct localhost*)ctx)->protocol == IPPROTO_UDP) {
        ...
        SOCKET_LOG(LOG_INFO, "UDP socket bound: fd=%d, ip=0x%x, port=%d", 
        ...
        SOCKET_LOG(LOG_INFO, "TCP socket bound: fd=%d, ip=0x%x, port=%d", 
                  sockfd, ntohl(saddr->sin_addr.s_addr), ntohs(saddr->sin_port));
    }
    return SOCKET_SUCCESS;
}
...
int listen(int sockfd, int backlog) {
    SOCKET_LOG(LOG_DEBUG, "Setting socket fd=%d to listen mode with backlog=%d", sockfd, backlog);
    ...
    if (stream->protocol != IPPROTO_TCP) {
        SOCKET_LOG(LOG_ERROR, "Operation not supported on non-TCP socket: fd=%d", sockfd);
        errno = EOPNOTSUPP;
        return SOCKET_ERROR_INVALID;
    }

    if (backlog < 0 || backlog > SOMAXCONN) {
        SOCKET_LOG(LOG_ERROR, "Invalid backlog value: %d", backlog);
        errno = EINVAL;
        return SOCKET_ERROR_INVALID;
    }

    stream->status = NG_TCP_STATUS_LISTEN;
    SOCKET_LOG(LOG_INFO, "Socket fd=%d now listening with backlog=%d", sockfd, backlog);
    return SOCKET_SUCCESS;
}
...
int accept(int sockfd, struct sockaddr *addr, socklen_t *addrlen) {
    SOCKET_LOG(LOG_DEBUG, "Accepting connection on socket fd=%d", sockfd);
    ...
    if (listener->status != NG_TCP_STATUS_LISTEN) {
        SOCKET_LOG(LOG_ERROR, "Socket fd=%d not in listen state", sockfd);
        errno = EINVAL;
        return SOCKET_ERROR_INVALID;
    }

    if (!addr || !addrlen) {
        SOCKET_LOG(LOG_ERROR, "Invalid address pointer or address length pointer");
        errno = EFAULT;
        return SOCKET_ERROR_INVALID;
    }
    ...
    while ((new_stream = tcp_get_pending_connection(listener)) == NULL) {
        if (non_blocking) {
            SOCKET_LOG(LOG_DEBUG, "No pending connection available (non-blocking mode)");
        ...
        if (current_time.tv_sec - start_time.tv_sec >= g_config.timeout_sec) {
            SOCKET_LOG(LOG_WARNING, "Accept operation timed out after %d seconds", g_config.timeout_sec);
            pthread_mutex_unlock(&listener->mutex);
            errno = ETIMEDOUT;
            return SOCKET_ERROR_TIMEOUT;
        }
    ...
    if (new_fd == -1) {
        SOCKET_LOG(LOG_ERROR, "Failed to allocate file descriptor for new connection");
        errno = EMFILE;
        return SOCKET_ERROR_NOMEM;
    }
    ...
    *addrlen = sizeof(struct sockaddr_in);
    ...
    SOCKET_LOG(LOG_INFO, "Accepted new connection: fd=%d, remote_ip=0x%x, remote_port=%d", 
...
ssize_t send(int sockfd, const void *buf, size_t len, int flags) {
    SOCKET_LOG(LOG_DEBUG, "Sending data on socket fd=%d, len=%zu, flags=0x%x", sockfd, len, flags);

    // 1. 参数验证
    if (!buf || len == 0) {
        SOCKET_LOG(LOG_ERROR, "Invalid buffer or length");
        errno = EINVAL;
        return SOCKET_ERROR_INVALID;
    }

    if (len > UINT16_MAX) {
        SOCKET_LOG(LOG_ERROR, "Data length exceeds maximum allowed size");
        errno = EMSGSIZE;
        return SOCKET_ERROR_INVALID;
    }
    ...
    if (((struct localhost*)ctx)->protocol == IPPROTO_UDP) {
        ...
        if (!ol) {
            SOCKET_LOG(LOG_ERROR, "Failed to allocate offload structure");
            errno = ENOMEM;
            return SOCKET_ERROR_NOMEM;
        }
        ...
        if (!ol->data) {
            SOCKET_LOG(LOG_ERROR, "Failed to allocate payload buffer");
            rte_free(ol);
            errno = ENOMEM;
            return SOCKET_ERROR_NOMEM;
        }
        ...
        if (non_blocking && rte_ring_full(udp_ctx->sndbuf)) {
            SOCKET_LOG(LOG_WARNING, "UDP send buffer full (non-blocking mode)");
            rte_free(ol->data);
            rte_free(ol);
            errno = EAGAIN;
        ...
        while (rte_ring_full(udp_ctx->sndbuf)) {
            ...
            if (current_time.tv_sec - start_time.tv_sec >= g_config.timeout_sec) {
                ...
                rte_free(ol->data);
                rte_free(ol);
                SOCKET_LOG(LOG_WARNING, "Send operation timed out after %d seconds", g_config.timeout_sec);
                errno = ETIMEDOUT;
                return SOCKET_ERROR_TIMEOUT;
            }
        ...
        if (rte_ring_mp_enqueue(udp_ctx->sndbuf, ol) < 0) {
            pthread_mutex_unlock(&udp_ctx->mutex);
            SOCKET_LOG(LOG_ERROR, "Failed to enqueue UDP data");
            rte_free(ol->data);
            rte_free(ol);
            errno = ENOSPC;
            return SOCKET_ERROR_SEND;
        ...
        pthread_mutex_unlock(&udp_ctx->mutex);
        SOCKET_LOG(LOG_INFO, "Successfully sent %zu bytes on UDP socket fd=%d", len, sockfd);
        return len;
        ...
        if (stream->status != NG_TCP_STATUS_ESTABLISHED) {
            SOCKET_LOG(LOG_ERROR, "TCP socket not in established state");
            errno = ENOTCONN;
            return SOCKET_ERROR_INVALID;
        }
        ...
        if (!frag) {
            SOCKET_LOG(LOG_ERROR, "Failed to allocate TCP fragment");
            errno = ENOMEM;
            return SOCKET_ERROR_NOMEM;
        }
        ...
        if (!frag->data) {
            SOCKET_LOG(LOG_ERROR, "Failed to allocate TCP payload buffer");
            rte_free(frag);
            errno = ENOMEM;
            return SOCKET_ERROR_NOMEM;
        }
        ...
        if (non_blocking && rte_ring_full(stream->sndbuf)) {
            SOCKET_LOG(LOG_WARNING, "TCP send buffer full (non-blocking mode)");
            rte_free(frag->data);
            rte_free(frag);
            errno = EAGAIN;
            return SOCKET_ERROR_SEND;
        ...
        if (rte_ring_mp_enqueue(stream->sndbuf, frag) < 0) {
            pthread_mutex_unlock(&stream->mutex);
            SOCKET_LOG(LOG_ERROR, "Failed to enqueue TCP data");
            rte_free(frag->data);
            rte_free(frag);
            errno = ENOSPC;
            return SOCKET_ERROR_SEND;
        ...
        pthread_mutex_unlock(&stream->mutex);
        SOCKET_LOG(LOG_INFO, "Successfully sent %zu bytes on TCP socket fd=%d", len, sockfd);
        return len;
    }

    SOCKET_LOG(LOG_ERROR, "Invalid protocol");
    errno = EINVAL;
    return SOCKET_ERROR_INVALID;
}
...
ssize_t recv(int sockfd, void *buf, size_t len, int flags) {
    SOCKET_LOG(LOG_DEBUG, "Receiving data on socket fd=%d, max_len=%zu, flags=0x%x", sockfd, len, flags);
    ...
    if (((struct localhost*)ctx)->protocol == IPPROTO_UDP) {
        ...
        if (non_blocking && rte_ring_empty(udp_ctx->rcvbuf)) {
            SOCKET_LOG(LOG_DEBUG, "No data available for UDP socket (non-blocking mode)");
            errno = EAGAIN;
            return SOCKET_ERROR_TIMEOUT;
        }
        ...
        while (rte_ring_mc_dequeue(udp_ctx->rcvbuf, (void**)&ol) < 0) {
            if (non_blocking) {
                pthread_mutex_unlock(&udp_ctx->mutex);
                SOCKET_LOG(LOG_DEBUG, "No data available for UDP socket");
                errno = EAGAIN;
                return SOCKET_ERROR_TIMEOUT;
            }
            ...
            if (current_time.tv_sec - start_time.tv_sec >= g_config.timeout_sec) {
                pthread_mutex_unlock(&udp_ctx->mutex);
                SOCKET_LOG(LOG_WARNING, "Receive operation timed out after %d seconds", g_config.timeout_sec);
                errno = ETIMEDOUT;
                return SOCKET_ERROR_TIMEOUT;
            }
        ...
        pthread_mutex_unlock(&udp_ctx->mutex);
        SOCKET_LOG(LOG_INFO, "Successfully received %zu bytes on UDP socket fd=%d", copy_len, sockfd);
        return copy_len;
        ...
        if (non_blocking && rte_ring_empty(stream->rcvbuf)) {
            SOCKET_LOG(LOG_DEBUG, "No data available for TCP socket (non-blocking mode)");
            errno = EAGAIN;
            return SOCKET_ERROR_TIMEOUT;
        }
        ...
        while (rte_ring_mc_dequeue(stream->rcvbuf, (void**)&frag) < 0) {
            if (non_blocking) {
                pthread_mutex_unlock(&stream->mutex);
                SOCKET_LOG(LOG_DEBUG, "No data available for TCP socket");
                errno = EAGAIN;
                return SOCKET_ERROR_TIMEOUT;
            }
        ...
        pthread_mutex_unlock(&stream->mutex);
        SOCKET_LOG(LOG_INFO, "Successfully received %zu bytes on TCP socket fd=%d", copy_len, sockfd);
        return copy_len;
    }
...
int close(int sockfd) {
    SOCKET_LOG(LOG_DEBUG, "Closing socket fd=%d", sockfd);
    ...
    if (((struct localhost*)ctx)->protocol == IPPROTO_UDP) {
        ...
        // 释放上下文
        rte_free(udp_ctx);
        SOCKET_LOG(LOG_INFO, "UDP socket fd=%d closed successfully", sockfd);
        ...
        // 释放上下文
        rte_free(stream);
        SOCKET_LOG(LOG_INFO, "TCP socket fd=%d closed successfully", sockfd);
    }
...
static int init_dpdk(void) {
    ...
    if (ret < 0) {
        SOCKET_LOG(LOG_ERROR, "Error with EAL init: %s", rte_strerror(rte_errno));
        return SOCKET_ERROR_INVALID;
    }
    ...
    g_mbuf_pool = rte_pktmbuf_pool_create("mbuf_pool", 
                                         ...
                                         rte_socket_id());
    if (!g_mbuf_pool) {
        SOCKET_LOG(LOG_ERROR, "Could not create mbuf pool: %s", rte_strerror(rte_errno));
        rte_eal_cleanup();
        return SOCKET_ERROR_NOMEM;
    }
    ...
    // 初始化环形缓冲区
    if (init_rings() != 0) {
        SOCKET_LOG(LOG_ERROR, "Failed to initialize rings");
    ...
    if (nb_ports == 0) {
        SOCKET_LOG(LOG_ERROR, "No supported ports found");
        rte_mempool_free(g_mbuf_pool);
    ...
    if (ret < 0) {
        SOCKET_LOG(LOG_ERROR, "Failed to get MAC address: %s", rte_strerror(-ret));
        rte_mempool_free(g_mbuf_pool);
        rte_ring_free(g_ring->in);
        rte_ring_free(g_ring->out);
        rte_free(g_ring);
    ...
    if (ret < 0) {
        SOCKET_LOG(LOG_ERROR, "Port configuration failed: %s", rte_strerror(-ret));
        rte_mempool_free(g_mbuf_pool);
        rte_ring_free(g_ring->in);
        rte_ring_free(g_ring->out);
        rte_free(g_ring);
    ...
    ret = rte_eth_rx_queue_setup(0, 0, 1024, 
                                ...
                                g_mbuf_pool);
    if (ret < 0) {
        SOCKET_LOG(LOG_ERROR, "RX queue setup failed: %s", rte_strerror(-ret));
        rte_eth_dev_close(0);
        rte_mempool_free(g_mbuf_pool);
        rte_ring_free(g_ring->in);
        rte_ring_free(g_ring->out);
    ...
    ret = rte_eth_tx_queue_setup(0, 0, 1024, 
                                ...
                                NULL);
    if (ret < 0) {
        SOCKET_LOG(LOG_ERROR, "TX queue setup failed: %s", rte_strerror(-ret));
        rte_eth_dev_close(0);
        rte_mempool_free(g_mbuf_pool);
        rte_ring_free(g_ring->in);
        rte_ring_free(g_ring->out);
    ...
    if (ret < 0) {
        SOCKET_LOG(LOG_ERROR, "Port start failed: %s", rte_strerror(-ret));
        rte_eth_dev_close(0);
        rte_mempool_free(g_mbuf_pool);
        rte_ring_free(g_ring->in);
        rte_ring_free(g_ring->out);
...
static int init_rings(void) {
    ...
    if (!g_ring) {
        SOCKET_LOG(LOG_ERROR, "Failed to allocate memory for ring structure: %s", 
    ...
    if (!g_ring->in) {
        SOCKET_LOG(LOG_ERROR, "Failed to create input ring: %s", 
                  rte_strerror(rte_errno));
        rte_free(g_ring);
        g_ring = NULL;
        return SOCKET_ERROR_NOMEM;
    ...
    g_ring->out = rte_ring_create("out_ring",
                                 ...
                                 RING_F_SP_ENQ | RING_F_SC_DEQ);
    if (!g_ring->out) {
        SOCKET_LOG(LOG_ERROR, "Failed to create output ring: %s", 
                  rte_strerror(rte_errno));
        rte_ring_free(g_ring->in);
        rte_free(g_ring);
        g_ring = NULL;
    ...
    SOCKET_LOG(LOG_INFO, "Ring buffers initialized successfully");
    return SOCKET_SUCCESS;
}
...
int socket_init(void) {
    ...
    if (pthread_mutex_init(&g_config_mutex, NULL) != 0) {
        SOCKET_LOG(LOG_ERROR, "Failed to initialize config mutex");
        return SOCKET_ERROR_INVALID;
    }
    ...
    ret = init_dpdk();
    if (ret != SOCKET_SUCCESS) {
        SOCKET_LOG(LOG_ERROR, "Failed to initialize DPDK");
    ...
    if (lcore_rx == RTE_MAX_LCORE || lcore_tx == RTE_MAX_LCORE) {
        SOCKET_LOG(LOG_ERROR, "Not enough CPU cores available");
        socket_cleanup();
        return SOCKET_ERROR_INVALID;
    }
    ...
    if (ret < 0) {
        SOCKET_LOG(LOG_ERROR, "Failed to launch RX thread on core %u: %s", 
    ...
    if (ret < 0) {
        SOCKET_LOG(LOG_ERROR, "Failed to launch TX thread on core %u: %s", 
    ...
    SOCKET_LOG(LOG_INFO, "Socket initialization completed successfully (RX on core %u, TX on core %u)", 
               lcore_rx, lcore_tx);
    return SOCKET_SUCCESS;
}
 ...
 */
static int rx_thread(__rte_unused void *arg) {
    SOCKET_LOG(LOG_INFO, "RX thread starting on core %u", rte_lcore_id());
    ...
    while (g_thread_running) {
        ...
        if (unlikely(nb_rx > BURST_SIZE)) {
            SOCKET_LOG(LOG_ERROR, "Error receiving from eth: received %u packets", nb_rx);
            continue;
        }
        ...
        if (likely(nb_rx > 0)) {
            ...
            if (unlikely(nb_enqueued < nb_rx)) {
                SOCKET_LOG(LOG_WARNING, "Failed to enqueue %u packets", nb_rx - nb_enqueued);
    ...
    SOCKET_LOG(LOG_INFO, "RX thread exiting from core %u", rte_lcore_id());
    return 0;
}
 ...
 */
static int tx_thread(__rte_unused void *arg) {
    SOCKET_LOG(LOG_INFO, "TX thread starting on core %u", rte_lcore_id());
    ...
    while (g_thread_running) {
        ...
        if (unlikely(diff_tsc > drain_tsc)) {
            ...
            if (total_tx > 0 || total_dropped > 0) {
                SOCKET_LOG(LOG_DEBUG, "TX stats: sent=%lu dropped=%lu", 
        ...
        if (likely(nb_dequeued > 0)) {
            ...
            if (unlikely(nb_tx < nb_dequeued)) {
                ...
                SOCKET_LOG(LOG_WARNING, "Failed to send %u packets", nb_dequeued - nb_tx);
    ...
    SOCKET_LOG(LOG_INFO, "TX thread exiting from core %u (total sent=%lu dropped=%lu)", 
...
void socket_cleanup(void) {
    SOCKET_LOG(LOG_INFO, "Starting socket cleanup...");

    // 1. 首先通知线程退出
    SOCKET_LOG(LOG_DEBUG, "Stopping threads...");
    g_thread_running = 0;

    // 等待所有线程完成
    RTE_LCORE_FOREACH_WORKER(lcore_id) {
        if (rte_eal_wait_lcore(lcore_id) < 0) {
            SOCKET_LOG(LOG_WARNING, "Failed to stop thread on core %u", lcore_id);
        }
    }
    SOCKET_LOG(LOG_INFO, "All threads stopped");

    // 2. 关闭所有打开的套接字
    SOCKET_LOG(LOG_DEBUG, "Closing open sockets...");
    pthread_mutex_lock(&g_config_mutex);
    for (int fd = 3; fd < g_config.max_fds; fd++) {
        int byte = fd / 8;
        int bit = fd % 8;
    ...
    // 3. 停止和关闭网络端口
    SOCKET_LOG(LOG_DEBUG, "Stopping network port...");
    rte_eth_dev_stop(0);
    rte_eth_dev_close(0);

    // 4. 清理ring资源
    SOCKET_LOG(LOG_DEBUG, "Cleaning up ring resources...");
    if (g_ring != NULL) {
        if (g_ring->in != NULL) {
            // 清空并释放入口ring
            void *msg;
    ...
    // 5. 释放内存池
    SOCKET_LOG(LOG_DEBUG, "Freeing memory pool...");
    if (g_mbuf_pool != NULL) {
        rte_mempool_free(g_mbuf_pool);
        g_mbuf_pool = NULL;
    }
    ...
    // 6. 清理基础设施
    SOCKET_LOG(LOG_DEBUG, "Cleaning up basic infrastructure...");
    pthread_mutex_destroy(&g_config_mutex);
    
    // 7. 最后清理DPDK环境
    SOCKET_LOG(LOG_DEBUG, "Cleaning up DPDK environment...");
    rte_eal_cleanup();

    // 8. 关闭日志系统
    SOCKET_LOG(LOG_INFO, "Socket module cleanup completed");
    closelog();
}
